...
The server also assigns one of two atomic quota slots to each secret-keyed
network fingerprint within a block. A retry with the same submission identifier
returns the original receipt; a third distinct submission from the same network
to the same block is rejected.
...
Sunucu ayrıca gizli anahtarla üretilmiş her ağ parmak izine bölüm içinde iki
atomik kota yuvasından birini verir. Aynı gönderim kimliğinin tekrarı ilk teslim
numarasını döndürür; aynı ağdan aynı bölüme üçüncü farklı gönderim reddedilir.
