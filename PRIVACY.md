...
- The application database does not store an IP address, user agent, real name
  or contact field. The hosting provider may still process technical connection
...
- Uygulama veritabanında IP adresi, tarayıcı bilgisi, gerçek ad veya iletişim
  alanı tutulmaz. Barındırma sağlayıcısı teknik bağlantı kayıtlarını kendi
  politikaları kapsamında işleyebilir.
